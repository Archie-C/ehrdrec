...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset, batch_size=256, shuffle=False, collate_fn=collate_patient_visit_histories)
    val_loader = DataLoader(val_dataset, batch_size=256, shuffle=False, collate_fn=collate_patient_visit_histories)
    ...
    def trial_fn(trial: optuna.Trial, train_loader: DataLoader, val_loader: DataLoader) -> Trainer:
        ...
        model = FastRx(
            ...
            embedding_dim_fastformer=embedding_dim_fastformer,
            dropout=dropout,
        )
    ...
    model = FastRx(
        ...
        embedding_dim_fastformer=best_params["embedding_dim_fastformer"],
    ...
    evaluator = Evaluator(
        model=model,
        test_loader=DataLoader(test_dataset, batch_size=256, shuffle=False, collate_fn=collate_patient_visit_histories),
        metrics=make_full_metrics(),
        device=device,
    )
