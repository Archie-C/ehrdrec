...
class LAMRec(nn.Module):
    def __init__(
        ...
        ddi_threshold: float = 0.06,
    ):
        super(LAMRec, self).__init__()
        self.diagnoses_embedding = nn.Embedding(n_diagnoses, embedding_dim, padding_idx=ReservedId.PAD)
        self.procedures_embedding = nn.Embedding(n_procedures, embedding_dim, padding_idx=ReservedId.PAD)
        
        self.alpha = alpha
        self.beta = beta
        self.ddi_threshold = ddi_threshold
        
        self.sequence_encoder = TransformerCrossAttention(
            d_model=embedding_dim,
            nhead=number_of_heads,
            num_layers=num_layers,
            dim_feedforward=feedforward_dim,
        )
        self.multi_view_contrastive_loss = MultiViewContrastiveLoss(temperature=temperature)
        self.label_wise_attention = LabelAttention(input_size=embedding_dim * 2, projection_size=embedding_dim, num_classes=n_medications)
        
        self.register_buffer("ddi_adjacency_matrix", ddi_adjacency_matrix.float())

    def forward(self, features):
        diagnoses, procedures = features['diagnoses'], features['procedures'] # [batch_size, num_visits, codes_per_visit]
        
        diagnoses_embedded = self.diagnoses_embedding(diagnoses)
        diagnoses_embedded = torch.sum(diagnoses_embedded, dim=2)
        mask = torch.any(diagnoses_embedded != 0, dim=2)
        procedures_embedded = self.procedures_embedding(procedures)
        procedures_embedded = torch.sum(procedures_embedded, dim=2)
        
        diagnoses_out, procedures_out = self.sequence_encoder(diagnoses_embedded, procedures_embedded, mask=mask)
        
        multiview_loss = self.multi_view_contrastive_loss(diagnoses_out, procedures_out)
        
        patient_representation = torch.cat([diagnoses_out, procedures_out], dim=-1)
        logits = self.label_wise_attention(patient_representation)
        
        if self.training:
            ddi_loss = self.compute_ddi_loss(logits)
            return {
                "predictions": logits,
                "losses" : {
                    "ddi_loss": ddi_loss,
    ...
    def compute_ddi_loss(self, logits):
        y_prob = torch.sigmoid(logits)
        y_pred = y_prob >= 0.5

        upper_mask = torch.triu(
            torch.ones_like(self.ddi_adjacency_matrix, dtype=torch.bool),
            diagonal=1,
        )
        pair_mask = y_pred.unsqueeze(2) & y_pred.unsqueeze(1)
        valid_pairs = pair_mask & upper_mask
        ddi_pairs = valid_pairs & self.ddi_adjacency_matrix.bool()

        total_pairs = valid_pairs.sum()
        if total_pairs == 0:
            current_ddi_rate = logits.new_zeros(())
        else:
            current_ddi_rate = ddi_pairs.sum().to(logits.dtype) / total_pairs.to(logits.dtype)

        if current_ddi_rate < self.ddi_threshold:
            return logits.new_zeros(())

        mul_pred_prob = y_prob.T @ y_prob
        return torch.sum(mul_pred_prob * self.ddi_adjacency_matrix) / self.ddi_adjacency_matrix.shape[0] ** 2
