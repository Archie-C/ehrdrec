...
from ehrdrec.models.torch.original.fastrx import FastRx
...
LR = 1e-3
EMB_DIM = 128
FASTFORMER_EMB_DIM = 128
DROPOUT = 0.1
DDI_WEIGHT = 0.05
SEED = 42


class FastRxTrainingAdapter(nn.Module):
    """Adapts padded multihot history batches to original FastRx inputs."""

    def __init__(self, model: FastRx) -> None:
        super().__init__()
        self.model = model
    ...
    def forward(self, features: dict[str, torch.Tensor]):
        ...
        for batch_idx in range(batch_size):
            ...
            sample_features = {
                "diagnoses": self._multihot_visits_to_ids(features["diagnoses"][batch_idx, :length]),
                "procedures": self._multihot_visits_to_ids(features["procedures"][batch_idx, :length]),
                "medications": self._multihot_visits_to_ids(features["medication_history"][batch_idx, :length]),
            }
    ...
    @staticmethod
    def _multihot_visits_to_ids(visits: torch.Tensor) -> list[torch.Tensor]:
        return [visit.nonzero(as_tuple=False).flatten() for visit in visits]
...
def build_fastrx_model(
    *,
    ...
    fastrx = FastRx(
        ...
        embedding_dim_fastformer=FASTFORMER_EMB_DIM,
        dropout=DROPOUT,
        device=device,
    )
    return FastRxTrainingAdapter(fastrx).to(device)
...
if __name__ == "__main__":
    ...
    logger = CompositeLogger(
        [
            TqdmLogger(epochs=EPOCHS, metrics=["Jaccard"], desc="FastRx"),
            CheckpointLogger(
                checkpoint_dir="checkpoints/fastrx",
                keep_last=True,
            ),
        ]
    )
