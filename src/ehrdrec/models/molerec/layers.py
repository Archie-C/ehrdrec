...
class AdjAttenAgger(nn.Module):
    """
    MoleRec adjacency-aware attention aggregator.

    This is kept structurally equivalent to the original implementation.
    """
    ...
    def forward(
        ...
        substructure_weights: torch.Tensor,
        ...
        weighted_substructures = torch.matmul(
            torch.diag(substructure_weights),
            other_features,
        )

        return torch.matmul(
            attention,
            weighted_substructures,
        )
