...
from ehrdrec.models.torch.original.rasnet import RASNet
...
class RASNetTrainingAdapter(nn.Module):
    """Adapts padded multihot history batches to original RASNet inputs."""

    def __init__(self, model: RASNet) -> None:
        super().__init__()
        self.model = model
    ...
    def _multihot_visits_to_ids(visits: torch.Tensor) -> list[torch.Tensor]:
        return [visit.nonzero(as_tuple=False).flatten().long() for visit in visits]
...
def build_rasnet_model(
    *,
    ...
    rasnet = RASNet(
        ...
        ddi_mask_H=ddi_adj.float(),
        fai=FAI,
        dropout=DROPOUT,
        embedding_dim=EMB_DIM,
        device=device,
    )
    return RASNetTrainingAdapter(rasnet).to(device)
...
if __name__ == "__main__":
    ...
    logger = CompositeLogger(
        [
            TqdmLogger(epochs=EPOCHS, metrics=["Jaccard"], desc="RASNet"),
            CheckpointLogger(
                checkpoint_dir="checkpoints/rasnet",
                keep_last=True,
            ),
        ]
    )
